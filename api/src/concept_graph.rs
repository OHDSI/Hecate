...
impl ConceptExpander {
    pub async fn expand(
        ...
        // Perform expansion
        let concept = get_concept_by_id(client, concept_id).await?;
        ...
        let concept_trees = if let Some(levels) = parent_levels {
            if levels > 0 {
                Self::build_parent_hierarchy(client, parents_cache, expand_concept, levels).await?
            } else {
                vec![expand_concept]
            }
        } else {
    ...
    async fn build_parent_hierarchy(
        ...
        Self::get_parents_recursive(client, parents_cache, concept, 1, max_levels, true).await
    }
