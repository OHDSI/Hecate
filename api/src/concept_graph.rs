...
impl<'a> ConceptExpander<'a> {
    ...
    pub async fn expand(
        ...
        if let Some(cached_response) = self.expand_cache.get(&cache_key).await {
            return Ok(cached_response);
        }

        let mut concept = get_concept_by_id(self.client, concept_id).await?;
        concept.record_count = self.record_counts.get(&concept_id).copied().unwrap_or(0);
        let mut expand_concept = ExpandConcept::from(concept);

        if let Some(levels) = child_levels
            && levels > 0
        {
            expand_concept = self.attach_children(expand_concept, levels).await?;
        }

        let concept_trees = if let Some(levels) = parent_levels {
            if levels > 0 {
                self.build_parent_hierarchy(expand_concept, levels).await?
            } else {
                vec![expand_concept]
            }
        } else {
        ...
        if elapsed.as_millis() > 1000 {
            ...
            self.expand_cache.insert(cache_key, response.clone()).await;
        }
                ...
                if current_level < max_levels {
                    let children =
                        get_concept_children_cached(client, children_cache, concept.concept_id)
                            .await?;
                    let expand_children: Vec<ExpandConcept> = children
                        .into_iter()
                        .map(|mut child| {
                            child.record_count =
                                record_counts.get(&child.concept_id).copied().unwrap_or(0);
                            ExpandConcept::from(child)
                        })
                        .collect();
    ...
    fn get_parents_recursive(
        ...
        Box::pin(async move {
            if max_levels == -1 || current_level < max_levels {
                ...
                let parents: Vec<_> = parent_concepts
                    .into_iter()
                    .filter(|parent| parent.vocabulary_id == concept.vocabulary_id)
                    .collect();

                if !parents.is_empty() {
                    let mut parent_expand_concepts = Vec::with_capacity(parents.len());
